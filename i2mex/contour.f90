subroutine i2mex_contour(nx, ny, x, y, f, x0, y0, xp, yp, ns, ttot, xs, ys, ier)
  ...
  allocate(dx(neq),stat=zsts)
  allocate(dxnew(neq),stat=zsts)
  allocate(xx(neq),stat=zsts)
  allocate(xx0(neq),stat=zsts)
  allocate(aa(nsteps),stat=zsts)
...
!!$   xx = (/xp, yp/)
  ...
  do while (npoints < nsteps-1 .and. abs(const*dt) > eps)
     ...
     if (a > twopi/2.) then
        print *, '?i2mex_contour: contour angle jumped by more than pi probably due to a flat spot'
        ier=1
        return
     end if
     angle = angle + a
