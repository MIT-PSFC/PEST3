!----------------------------------------------------------
! r8real -- generated (f90 fixed form) REAL interface to:  EQM_TBDY
      SUBROUTINE R4_EQM_TBDY(
     > ILINES,R4_RL,R4_ZL,R4_THL,ICIRCS,R4_RC,R4_ZC,R4_RAD,IERR)
...
! allocation of working arrays...
     ...
     >  ' ?R4_EQM_TBDY -- RL_act ALLOCATE error!')
      ALLOCATE(ZL_act(ILINES)
     >   ,stat=alloc_stat)
      if(alloc_stat.ne.0) call errmsg_exit(
     >  ' ?R4_EQM_TBDY -- ZL_act ALLOCATE error!')
      ALLOCATE(THL_act(ILINES)
     >   ,stat=alloc_stat)
      if(alloc_stat.ne.0) call errmsg_exit(
     >  ' ?R4_EQM_TBDY -- THL_act ALLOCATE error!')
      ALLOCATE(RC_act(ICIRCS)
     >   ,stat=alloc_stat)
      if(alloc_stat.ne.0) call errmsg_exit(
     >  ' ?R4_EQM_TBDY -- RC_act ALLOCATE error!')
      ALLOCATE(ZC_act(ICIRCS)
     >   ,stat=alloc_stat)
      if(alloc_stat.ne.0) call errmsg_exit(
     >  ' ?R4_EQM_TBDY -- ZC_act ALLOCATE error!')
      ALLOCATE(RAD_act(ICIRCS)
     >   ,stat=alloc_stat)
      if(alloc_stat.ne.0) call errmsg_exit(
     >  ' ?R4_EQM_TBDY -- RAD_act ALLOCATE error!')
...
! executable code:  copy for input
      ...
      ZC_act=R4_ZC
      RAD_act=R4_RAD
! call to original routine:  EQM_TBDY
