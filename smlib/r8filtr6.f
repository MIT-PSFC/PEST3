...
C-----------------------------------------------------------------------
C
      SUBROUTINE r8filtr6(X,Y1,Y,N,EPS,NE,EPS2,NEPS2,DELT,IDDF,DDV,
...
C idecl:  explicitize implicit INTEGER declarations:
      ...
      INTEGER n,neps2,iddf,iblf,ndrop,nblrs,ierr,j,ierr1,j2,n2,je,nel,ne
C============
C idecl:  explicitize implicit REAL declarations:
      REAL*8 y1,y,eps,eps2,delt,ddv,blv,xenda,xendb,sm,x,zxp,xend1,
     > xend2,zeps,y1a,y1b,zmax,dy
C============
      DIMENSION X(N),Y1(N),Y(N),EPS2(*)  !EPS2 IS AN ARRAY
...
C  DROP DATA PTS IN Y1 ARRAY
      IF(IDDF.EQ.1) CALL r8ddrop(X,Y1,N,DDV,NDROP)
C
      IERR=0
      ZXP=X(1)-(X(2)-X(1))
      DO 10 J=1,N
...
C
      XEND1=XENDA
      XEND2=XENDB
      CALL r8filter(X,Y1,Y,N,DELT,XEND1,XEND2)
...
C  IMPLEMENT EPSLON FEATURE --
      CALL r8fileps(X,Y1,Y,N,DELT,EPS,NE,EPS2,NEPS2,XEND1,XEND2)
C
      GO TO 500
C
...
CTES	CALL DATCPU(ZTIM1)
      CALL r8filter(X(J),Y1(J),Y(J),N2,DELT(J),XEND1,XEND2)
...
CT 9902	FORMAT(' FILEPS   J,J2:',2X,I4,2X,I4)
      CALL r8fileps(X(J),Y1(J),Y(J),N2,DELT(J),EPS(JE),NEL,
...
C
      ZMAX=0.0E0_R8
      DO 505 J=1,N
      ZMAX=max(ZMAX,ABS(Y1(J)))
 505  CONTINUE
      IF(ZMAX.EQ.0.0E0_R8) ZMAX=1.0E0_R8
C
      DO 510 J=1,N
      DY=(Y(J)-Y1(J))/ZMAX
