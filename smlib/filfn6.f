...
C---------------------------------------------
C
      REAL FUNCTION FILFN6(X,Y,N,J0,DELTA0,XEND1,XEND2,ICOD)
C
...
C  CHECK FOR NUMERICALLY ZERO OR NEGATIVE DELTA
      DELTA=AMIN1((X(N)-X(1)),DELTA0(J0))
      IF((DELTA.LE.0.0).OR.((X(J0)+DELTA).EQ.X(J0))) THEN
        IF(ICOD.EQ.1) THEN
          FILFN6=Y(J0)
        ELSE
          FILFN6=0.0
...
C  CHECK FOR FIXED ENDPOINTS
        FILFN6=Y(J0)
        IF((J0.EQ.1).AND.(XEND1.EQ.0.0)) RETURN
        IF((J0.EQ.N).AND.(XEND2.EQ.0.0)) RETURN
      ENDIF
C
...
C
 15   CONTINUE
      ...
      WASUM=WASUM+ZSIGN*WXFINT(XA1,XB,XA,YA,XB,YB,X(J0),DELTA)
C
      GO TO 10
C
...
C
      WASUM=WASUM+ZSIGN*WXFINT(XA,XB1,XA,YA,XB,YB,X(J0),DELTA)
C
      GO TO 60
C
C
