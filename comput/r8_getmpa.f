...
! idecl:  explicitize implicit REAL declarations:
      REAL*8 zth,zr,zy
!============
      REAL*8 zrmc(0:mimom,2)  ! R moments
      REAL*8 zymc(0:mimom,2)  ! Y moments
C
C  mimom gives the moments 1st array dimension; imom gives the number
C   of *active* moments; imom.le.mimom ...
C
      REAL*8 zrin            ! inner intercept R (output)
      REAL*8 zrout           ! outer intercept R (output)
      REAL*8 zymp            ! midplane vertical displacement (output)
C
      parameter (inth=400)
      REAL*8 zrcon(inth),zycon(inth)
      REAL*8 zcos(0:imom),zsin(0:imom)
C---------------------------------------------------------------------
C
      zcos(0)=1
      zsin(0)=0
      do ith=1,inth-1
        zth=(6.2831853071795862D+00*(ith-1))/(inth-1)
        zr=0.0D0
        zy=0.0D0
...
C
      ...
      real*8, intent(in) :: zrcon(inth),zycon(inth)  ! closed contour

      real*8, intent(out) :: zrin,zrout    ! midplane intercepts (approx.)
      real*8, intent(out) :: zymp          ! midplane elevation (centroid)
C
C  local:
C
      real*8 :: zrmp,zr,zy,zrp,zyp,zytest,zrans
      integer :: ith
C
C  find centroid
C
